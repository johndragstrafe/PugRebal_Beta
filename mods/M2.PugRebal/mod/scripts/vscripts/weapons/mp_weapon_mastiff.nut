...
struct {
	float[2][MASTIFF_MAX_BOLTS*2] boltOffsets = [
		[0.0, 0.15], //
		[0.0, 0.3], //
		[0.0, 0.6], //
		[0.0, 1.2], //
...
function FireWeaponPlayerAndNPC( WeaponPrimaryAttackParams attackParams, bool playerFired, entity weapon )
{
	...
	if ( shouldCreateProjectile )
	{
        ...
        int bolts = MASTIFF_MAX_BOLTS
        if (weapon.HasMod("double_pellets")) {
            bolts *= 2
        }
		for ( int index = 0; index < bolts; index++ )
		{
			...
			if ( bolt != null )
			{
				...
				#endif
                if (index >= MASTIFF_MAX_BOLTS) { // doubled pellet
